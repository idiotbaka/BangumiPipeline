...
404 的 `(musume_97)`、`(musume_98)` 会固定跳过。完整的 `manifest.json` 会作为持久
完成标记，后续自动任务不会每分钟重新遍历图片；需要检查或修复本地文件时可重新执行
手动同步脚本。计划任务自动下载时会使用系统设置中保存的 HTTP/HTTPS 代理。

历史成品评论回填每轮最多发现和同步 10 话，回填耗尽后会将完成标记持久化，后续每分钟
任务直接跳过历史扫描。每个成品还会记录评论入队状态，服务重启时仅通过“未入队成品”
部分索引检查异常中断窗口；新成品仍会在媒体处理完成时立即入队，入队失败会自动重新开启兜底。
