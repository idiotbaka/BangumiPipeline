...
func (s *Service) IsInitialized(ctx context.Context) (bool, error) {
	var exists bool
	err := s.db.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM users WHERE is_admin = 1)").Scan(&exists)
	return exists, err
}

func (s *Service) Setup(ctx context.Context, username, password string) (User, Session, error) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM users WHERE is_admin = 1)").Scan(&initialized); err != nil {
		return User{}, Session{}, err
	}
	if initialized {
	...
	result, err := tx.ExecContext(ctx,
		...
		username, string(passwordHash), now,
	)
	if err != nil {
		return User{}, Session{}, fmt.Errorf("create administrator: %w", err)
	}
	...
	return User{ID: userID, Username: username, IsAdmin: true, CreatedAt: now}, session, nil
}
...
WHERE sessions.token_hash = ? AND sessions.expires_at > ?`, tokenHash[:], s.now().UTC().Unix()).
...
func (s *Service) DeleteExpiredSessions(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM sessions WHERE expires_at <= ?", s.now().UTC().Unix())
	return err
}
