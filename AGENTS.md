...
- `viewer_comment_username_filters`：观看端吐槽过滤名单；同时按 Bangumi `username` 或 `nickname` 全匹配。
...
- 吐槽过滤名单只在管理端“网站设置”中维护，观看端评论列表与分集评论数量都必须排除 `username` 或 `nickname` 完全匹配 `viewer_comment_username_filters.username` 的记录；不要把过滤名单暴露给公开站点设置接口。
...
- APP 名称默认为 `BakaVip2`，版本默认为 `1.2.0`，包名为 `vip.baka.bangumipipeline.mobile`。
