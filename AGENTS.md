...
- `viewer_comment_username_filters`：观看端吐槽过滤用户名；按 Bangumi `username` 全匹配。
...
- 吐槽用户名过滤只在管理端“网站设置”中维护，观看端评论列表与分集评论数量都必须排除完全匹配 `viewer_comment_username_filters.username` 的记录；不要把过滤名单暴露给公开站点设置接口。
