...
- 服务初始化顺序：SQLite -> 日志 handler -> auth/system/bangumi/subscription/download/media -> scheduler -> HTTP servers。
- 计划任务注册点在 `main.go`：
...
旧 `AB_*` 变量只作为兼容 fallback，新代码和文档不要把 `AB_*` 作为首选。不要提交个人代理地址、账号、Cookie、Bangumi Token 或 qBittorrent 密码。
...
- `system_logs`：持久化系统日志。
- `anime_metadata`、`anime_aliases`、`anime_tags`、`anime_characters`、`actors`、`character_actors`：Bangumi 元数据。
...
- `media_jobs`：下载完成后的视频识别、转码/压制、最终产物路径和处理状态。
...
- 详情与角色阶段相互独立：一个阶段成功后，不应因另一个阶段失败而重复抓取。
...
番剧目录查询在 `backend/internal/bangumi/catalog.go`。番剧管理卡片的已绑定话数来自 `subscription_items`，如果对应 `media_jobs.status = completed`，该话数 `status` 返回 `completed`，前端显示绿色 tag；否则返回 `matched`，显示黄色 tag。
...
```text
data/bangumi/{番剧名称}/{Season 1|OVA|OAD|SP}/{番剧名称 S01E01.mp4}
```
...
- 业务代码使用 `log/slog`。
- 日志必须带 `source` 属性，例如 `bangumi`、`subscription`、`download`、`media`、`scheduler`、`http`。
...
- `frontend/apps/admin/src/pages/SettingsPage.vue`：网络、下载、订阅设置。
- `frontend/apps/admin/src/pages/ScheduledTasksPage.vue`：计划任务。
- `frontend/apps/admin/src/pages/AnimeListPage.vue`：番剧管理卡片、历史话数同步入口。
...
- 不直接向前端暴露本地绝对路径；页面只展示文件名、状态、错误摘要等。
...
不要把个人代理地址、账号、Cookie、Bangumi Token 或 qBittorrent 凭据提交到仓库。
