...
class PlayerPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    fun copyText(invoke: Invoke) {
        ...
        activity.runOnUiThread {
            try {
                ...
                clipboard.setPrimaryClip(ClipData.newPlainText("邀请码", args.text))
                invoke.resolve(JSObject())
            } catch (error: Exception) {
                invoke.reject("无法复制邀请码，请重试")
            }
        }
    }
    ...
    fun setTheme(invoke: Invoke) {
        ...
        activity.runOnUiThread {
            AppTheme.save(activity, args.dark)
            if (!fullscreenActive) AppTheme.applySystemBars(activity)
            invoke.resolve(JSObject())
        }
    }
    ...
    private fun restoreNormalSystemBars() {
        AppTheme.applySystemBars(activity)
    }
